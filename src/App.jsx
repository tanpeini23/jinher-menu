...
function DrinkModal({ onSelect, onClose, discount = 0 }) {
    ...
    return (
      <div style={LS.page}>
        ...
        <div style={{overflowY:"auto",flex:1,padding:"0 14px 120px"}}>
          ...
          {MENU[activeCat].items.filter(item=>!(group.disabledItems||[]).includes(item.id)&&!offSet.has(item.id)).map(item=>(
      ...
      {setMealPicking&&!drinkModal&&(
        ...
        {drinkModal&&<DrinkModal discount={80}
    ...
    <div style={LS.page}>
      ...
      <div style={{...LS.header,paddingBottom:"8px"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
          <div style={{fontSize:"12px",color:"#8a6a48"}}>{guestName}</div>
          <div style={{fontSize:"9px",color:"#c8b49a"}}>v82</div>
        </div>
      </div>
      <div style={{display:"flex",overflowX:"auto",padding:"0 12px 10px",gap:"6px"}}>
        {[...FOOD_CATS,...DRINK_CATS].map(k=>(
...
function ComplaintPanel({ g, setGroups, groups }) {
    ...
    <div style={{...S.page,background:"#f5f0e8",color:"#3a2a1a"}}>
      ...
      <div style={{...S.header,paddingBottom:"10px"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"10px"}}>
          <div style={S.logo}>✦ 大訂追蹤表 v82</div>
        ...
        {showHelp&&(
          <div style={{background:"#fffaf0",border:"1.5px solid #d8c090",borderRadius:"12px",padding:"12px 14px",marginBottom:"10px"}}>
            <div style={{fontSize:"13px",fontWeight:"800",color:"#8a5210",marginBottom:"8px"}}>📖 各按鍵用途</div>
            {[
              ...
              ["人數統計表","訂位人數統計表。每週一三五要導入訂位、關滿20的紅色時段。裡面有步驟 1-2-3 指引。"],
              ["🚫 品項","按日期關閉某些餐點(可選幾號到幾號),那天的客人就點不到。搜尋菜名即可。"],
              ["+ 新增大訂","手動新增一筆大訂訂位(客人沒線上訂、或電話訂位時用)。"],
            ].map(([k,v])=>(
              <div key={k} style={{display:"flex",gap:"8px",marginBottom:"6px",fontSize:"12px",lineHeight:"1.6"}}>
                <span style={{fontWeight:"800",color:"#6a4a2e",flexShrink:0,minWidth:"70px"}}>{k}</span>
                <span style={{color:"#5a4530"}}>{v}</span>
              </div>
            ))}
            <div style={{fontSize:"11px",color:"#8a6a4a",marginTop:"6px",borderTop:"1px solid #e8dcc0",paddingTop:"6px"}}>下方每組點餐狀態格可點:選「已封存/現場點餐」等。過期的訂位會出現「直接封存 / 客訴與建議」。</div>
          </div>
        )}
        <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
          <button onClick={()=>setShowDingwe(true)} style={{padding:"11px 14px",borderRadius:"9px",border:"none",background:"#dce8f4",color:"#1a4a6a",fontSize:"15px",fontWeight:"700",cursor:"pointer",whiteSpace:"nowrap"}}>人數統計表</button>
                ...
                <tr key={g.id} style={{background:rowBg(g),opacity:g.cancelled?0.55:(isPastMeal(g)&&!g.archived?0.6:1),borderBottom:"1px solid #f0e8d6"}}>
                  ...
                  <td style={{padding:"5px 6px",borderRight:"1px solid #e0d5c0",textAlign:"center"}}>
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"3px"}}>
                      ...
                      <button onClick={()=>setGroups(p=>p.map(x=>x.id!==g.id?x:{...x,locked:!(x.locked||isPastDeadline(x.date)),unlockOverride:(x.locked||isPastDeadline(x.date))?true:false}))}
                ...
                {expanded===g.id&&(
                  <tr key={g.id+"-exp"}>
                    <td colSpan={COLS.length+4} style={{background:"#faf6ee",padding:"8px 10px",borderBottom:"2px solid #c8b89c"}}>
                      <ComplaintPanel g={g} setGroups={setGroups} groups={groups}/>
...
function DingwePage({ groups, onBack, staffList, setGroups }) {
  ...
  const confirmImport = (staffName, forceUpdate=false) => {
    ...
    setPeopleMap(p=>{
      ...
      persistDW(n,closeMap,progress,importStaff.dupWarn||[],todayLI);
      return n;
    });
    setDupReservations(importStaff.dupWarn||[]);
  ...
  const guardedBack = () => {
    if(isCloseDay && unclosedRedCnt>0 && !leaveAck){ setLeaveWarn(true); }
    else onBack();
  };
  ...
  return (
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:"#f5f0e8",fontFamily:"'Noto Sans TC',sans-serif"}}>
      ...
      {/* 關訂位選夥伴 */}
      {closePicker&&<StaffPicker staffList={staffList} onSelect={n=>{setCloseMap(p=>{const m={...p,[closePicker]:n};persistDW(peopleMap,m,progress);return m;});setClosePicker(null);}} onClose={()=>setClosePicker(null)}/>}
      ...
      {leaveWarn&&createPortal(
        <div style={{position:"fixed",inset:0,zIndex:9000,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.7)",padding:"18px"}} onClick={()=>setLeaveWarn(false)}>
          <div style={{background:"#fff",borderRadius:"18px",padding:"22px",width:"100%",maxWidth:"340px",textAlign:"center",boxShadow:"0 12px 40px rgba(0,0,0,0.4)"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{fontSize:"17px",color:"#c02020",fontWeight:"800",marginBottom:"8px"}}>還有紅色訂位沒關!</div>
            <div style={{fontSize:"14px",color:"#5a4530",lineHeight:"1.7",marginBottom:"16px"}}>
              今天(週{["日","一","二","三","四","五","六"][_dowNow]})還有 <b style={{color:"#c02020"}}>{unclosedRedCnt} 個滿20人的時段</b>沒關訂位。<br/>要先去大麥POS關掉、並在這裡按「完成關訂位」嗎?
            </div>
            ...
            <div style={{display:"flex",gap:"8px"}}>
              <button onClick={()=>{setLeaveWarn(false);onBack();}}
                style={{flex:1,padding:"11px",borderRadius:"10px",border:"1px solid #d0b090",background:"#fbf3e6",color:"#8a5a10",fontSize:"12px",fontWeight:"700",cursor:"pointer"}}>稍後提醒關閉</button>
              <button onClick={()=>{setLeaveAck(true);setLeaveWarn(false);onBack();}}
                style={{flex:1,padding:"11px",borderRadius:"10px",border:"1px solid #c8b89c",background:"#f0e8d8",color:"#7a6a4a",fontSize:"12px",fontWeight:"700",cursor:"pointer"}}>確認不關閉</button>
            </div>
          </div>
        </div>,
        document.body
      ...
      <div className="np" style={{padding:"6px 12px",background:"#ede2d0",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
        ...
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:"13px",fontWeight:"700",color:"#6a4a2e"}}>✦ 訂位人數統計表 v82</div>
      ...
      {dupReservations&&dupReservations.length>0&&(
        <div className="np" style={{padding:"5px 12px",background:"#fbe8d8",borderBottom:"1px solid #e0b088",flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:dupOpen?"4px":"0",gap:"8px"}}>
            <span style={{fontSize:"12px",color:"#b05a10",fontWeight:"700",flex:1}}>⚠ 重複訂位 {dupReservations.length} 組{dupOpen?" — 按 ✕ 移除多訂的,人數自動更新":""}</span>
            ...
            <button onClick={()=>{setDupReservations([]);persistDW(peopleMap,closeMap,progress,[]);}} style={{fontSize:"10px",background:"#e0c0a0",border:"none",borderRadius:"5px",padding:"5px 8px",color:"#7a4a10",fontWeight:"700",cursor:"pointer",whiteSpace:"nowrap"}}>全部已確認</button>
          </div>
          {dupOpen&&dupReservations.map((d,di)=>(
  ...
  return (
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:"#f5f0e8",fontFamily:"'Noto Sans TC',sans-serif"}}>
      ...
      <div style={{padding:"10px 14px",background:"#ede2d0",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:"#6a4a2e",fontSize:"14px",cursor:"pointer",fontWeight:"700"}}>← 返回</button>
        <div style={{fontSize:"13px",fontWeight:"700",color:"#6a4a2e"}}>📊 數據統計 v82</div>
  ...
  if(!showRefund) return (
    <div style={{padding:"14px 16px",borderTop:"1px solid #e0d5c0",background:"#f5efe2"}}>
      <button onClick={()=>setShowRefund(true)}
        style={{width:"100%",padding:"12px",borderRadius:"12px",background:"#e4ecf4",border:"1px solid #3a5a7a",color:"#2a5a8a",fontSize:"13px",fontWeight:"700",cursor:"pointer"}}>
        💰 退款簽名（需員工密碼）
      </button>
    </div>
  );
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{...S.header, paddingBottom:"10px"}}>
        ...
        <div style={S.logo}>✦ 全組訂單總覽</div>
        <div style={{fontSize:"11px",color:"#aa8060"}}>{group.name} · {group.date} {group.time}</div>
      </div>
      <div style={{overflowY:"auto",flex:1,padding:"14px"}}>
        {group.archiveType==="menu"&&(
      ...
      <div style={{padding:"10px 16px 0",background:"#f5efe2"}}>
        <button onClick={()=>setHdPin("")}
          style={{width:"100%",padding:"12px",borderRadius:"12px",background:"#e2f0e2",border:"1px solid #9ac09a",color:"#2a6a3a",fontSize:"13px",fontWeight:"700",cursor:"pointer"}}>
          🍽 夥伴劃單（需員工密碼）
        </button>
      </div>
      {fromStaff&&(<>
      <div style={{padding:"10px 16px 0",background:"#f5efe2"}}>
        <button onClick={()=>setAddPin("")}
          style={{width:"100%",padding:"12px",borderRadius:"12px",background:"#f0e4f4",border:"1px solid #c0a0d0",color:"#6a3a8a",fontSize:"13px",fontWeight:"700",cursor:"pointer"}}>
          ➕ 員工新增客人訂單 / 客製餐點（需員工密碼）
        </button>
      </div>
      <div style={{padding:"10px 16px 0",background:"#f5efe2"}}>
        <button onClick={()=>{ if(staffMode) setStaffMode(false); else setSmPin(""); }}
          style={{width:"100%",padding:"12px",borderRadius:"12px",background:staffMode?"#dfeadf":"#e4ecf4",border:`1px solid ${staffMode?"#7ab87a":"#a8b8c8"}`,color:staffMode?"#1a6a3a":"#2a5a8a",fontSize:"13px",fontWeight:"700",cursor:"pointer"}}>
          {staffMode?"🔧 夥伴模式：開（可改素湯，點此關閉）":"🔧 夥伴模式（改素湯，需員工密碼）"}
        </button>
      </div>
      </>)}
      {smPin!==null&&(
      ...
      <RefundSection group={group} S={S} onSaveSig={(type,dataUrl,time)=>{
